...
public class CapacitorUpdaterPlugin: CAPPlugin {
    ...
    private var defaultDecryptStrategy: DecryptStrategy = DecryptStrategy("private", defaultPrivateKey);
    ...
    override public func load() {
        ...
        implementation.privateKey = self.defaultPrivateKey;
        implementation.hasOldPrivateKeyPropertyInConfig = false;
        if (getConfig().getString("privateKey") != null && !getConfig().getString("privateKey").isEmpty()) {
            implementation.privateKey = getConfig().getString("privateKey");
            implementation.hasOldPrivateKeyPropertyInConfig = true;
        }
        implementation.decryptStrategy = defaultDecryptStrategy;
        implementation.hasDecryptStrategyInConfig = false;
        if (getConfig().getObject("decryptStrategy") != null) {
            implementation.decryptStrategy = try JSONDecoder().decode(DecryptStrategy.self, from: getConfig().getObject("decryptStrategy").toString()!) as? DecryptStrategy
            this.implementation.hasDecryptStrategyInConfig = true;
        }
